...
public class InfoClient implements IInfoClient {
	...
	public ArrayList<FileLocation> getDirectory(INotifiableManager manager, String path, DirectoryMask mask,
												int offset, int limit, int mediaType) {
	...
	private ArrayList<String> getNonAddonDirectory(INotifiableManager manager, String path, DirectoryMask mask,
												   int offset, int limit) {
		return mConnection.getArray(manager, "GetDirectory", path + ";" + (mask != null ? mask.toString() : " ") + ";"
				+ (offset > 0 ? offset : " ") + ";"
				+ (limit > 0 ? limit : " "));
	}
	...
	public boolean getGuiSettingBool(INotifiableManager manager, int field) {
		return mConnection.getBoolean(manager, "GetGuiSetting", GuiSettings.getType(field) + ";" + GuiSettings.getName
				(field));
	}
	...
	public int getGuiSettingInt(INotifiableManager manager, int field) {
		return mConnection.getInt(manager, "GetGuiSetting", GuiSettings.getType(field) + ";" + GuiSettings.getName
				(field));
	}
	...
	public boolean setGuiSettingBool(INotifiableManager manager, int field, boolean value) {
		return mConnection.getBoolean(manager, "SetGuiSetting", GuiSettings.getType(field) + ";" + GuiSettings.getName
				(field) + ";" + value);
	}
