...
abstract class Client {
	...
	protected Bitmap getCover(INotifiableManager manager, ICoverArt cover, int size, String url, String fallbackUrl) {
		try {
			...
			manager.onError(new Exception("Out of memory. We're aware of this problem and we're working on it. " +
					"Restarting the app should help."));
			return null;
		}
	}
	 ...
	 * @return Bitmap
	 */
	private Bitmap getCoverFromLibGoAhead(INotifiableManager manager, ICoverArt cover, int size, String url,
										  String fallbackUrl) {
	 ...
	 * @return Bitmap
	 */
	private Bitmap getCoverFromMicroHTTPd(INotifiableManager manager, ICoverArt cover, int size, String url,
										  String fallbackUrl) {
		final int mediaType = cover.getMediaType();
		// don't fetch small sizes
		size = size < ThumbSize.BIG ? ThumbSize.MEDIUM : ThumbSize.BIG;
