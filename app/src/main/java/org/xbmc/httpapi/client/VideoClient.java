...
public class VideoClient extends Client implements IVideoClient {
	...
	private static final String MOVIE_COLUMNS = "idMovie, c00, c07, strPath, strFileName, c15, c11, c14, ROUND(c05, " +
			"2), playCount, c09, art.url";
	...
	public ArrayList<Movie> getMovies(INotifiableManager manager, int sortBy, String sortOrder, boolean hideWatched) {
		...
		sb.append(" WHERE movie.idFile=files.idFile AND path.idPath=files.idPath AND movie.idMovie=art.media_id AND " +
				"art.media_type='movie' AND art.type='thumb'");
	...
	public ArrayList<Movie> getMovies(INotifiableManager manager, Actor actor, int sortBy, String sortOrder,
		...
		sb.append(WHERE_MOVIES);
		sb.append(" WHERE movie.idFile=files.idFile AND path.idPath=files.idPath AND movie.idMovie=art.media_id AND " +
				"art.media_type='movie' AND art.type='thumb' AND movie.idmovie IN (");
	...
	public Movie updateMovieDetails(INotifiableManager manager, Movie movie) {
		...
		sb.append(WHERE_MOVIES);
		sb.append(" WHERE movie.idFile=files.idFile AND path.idPath=files.idPath AND movie.idMovie=art.media_id AND " +
				"art.media_type='movie' AND art.type='thumb' AND movie.idmovie = ");
		...
		sb.append(" FROM actors LEFT OUTER JOIN art ON art.media_id=idActor AND art.media_type='actor' and art" +
				".type='thumb', actorlinkmovie");
	 ...
	 * {@link #getTvShowActors} for filtered actors.
	 *
	 * @return All actors
	 */
	...
	private ArrayList<Movie> parseMovies(String response) {
		...
		try {
			for (int row = 1; row < fields.length; row += 12) { //WHen adding a field, be sure to change this #
				movies.add(new Movie( // int id, String title, int year, String path, String filename,
						// String director, String runtime, String genres, Double rating, int numWatched, String imdbId
						Connection.trimInt(fields[row]),
						Connection.trim(fields[row + 1]),
	...
	private String moviesOrderBy(int sortBy, String sortOrder) {
		switch (sortBy) {
			default:
			case SortType.TITLE:
				return " ORDER BY CASE WHEN c10 IS NULL OR c10 = '' THEN replace(lower(c00),'the ','') ELSE replace" +
						"(lower(c10),'the ','') END " + sortOrder;
			case SortType.YEAR:
				return " ORDER BY c07 " + sortOrder + ", CASE WHEN c10 IS NULL OR c10 = '' THEN lower(c00) ELSE " +
						"lower" +
						"(c10) END " + sortOrder;
	...
	/**
	 * Returns the first videos of the playlist.
	 *
