...
public class MusicClient extends Client implements IMusicClient {
	...
	public boolean addToPlaylist(INotifiableManager manager, Album album, int sortBy, String sortOrder) {
		return mConnection.getBoolean(manager, "AddToPlayListFromDB", LIBRARY_TYPE + ";" + getSongsCondition(album) +
				songsOrderBy(sortBy, sortOrder));
	}
	...
	public boolean addToPlaylist(INotifiableManager manager, Artist artist, int sortBy, String sortOrder) {
		return mConnection.getBoolean(manager, "AddToPlayListFromDB", LIBRARY_TYPE + ";" + getSongsCondition(artist) +
				songsOrderBy(sortBy, sortOrder));
	}
	...
	public boolean addToPlaylist(INotifiableManager manager, Genre genre, int sortBy, String sortOrder) {
		return mConnection.getBoolean(manager, "AddToPlayListFromDB", LIBRARY_TYPE + ";" + getSongsCondition(genre) +
				songsOrderBy(sortBy, sortOrder));
	}
	...
	public boolean addToPlaylist(INotifiableManager manager, Artist artist, Genre genre, int sortBy,
								 String sortOrder) {
		return mConnection.getBoolean(manager, "AddToPlayListFromDB", LIBRARY_TYPE + ";" + getSongsCondition(artist,
				genre) + songsOrderBy(sortBy, sortOrder));
	}
	...
	public boolean play(INotifiableManager manager, Artist artist, Genre genre) {
		return play(manager, getSongsCondition(artist, genre).append(songsOrderBy(SortType.ARTIST,
				SortType.ORDER_ASC)));
	}
	...
	public ArrayList<Album> getAlbums(INotifiableManager manager, ArrayList<Integer> artistIDs) {
		...
		sb.append("SELECT albumview.idAlbum, strAlbum, group_concat(DISTINCT strArtist) AS strArtists, iYear, " +
				"art.url");
		...
		sb.append(" LEFT OUTER JOIN art ON art.media_id=albumview.idAlbum AND art.media_type='album' AND art" +
				".type='thumb'");
	...
	public ArrayList<Genre> getGenres(INotifiableManager manager) {
		return parseGenres(mConnection.query("QueryMusicDatabase", "SELECT idGenre, " +
				"strGenre FROM genre ORDER BY upper" +
				"(strGenre), strGenre", manager));
	}
	 ...
	 * @return Found tracks
	 */
	private ArrayList<Song> getSongs(INotifiableManager manager, StringBuilder sqlCondition, int sortBy,
									 String sortOrder) {
		StringBuilder sb = new StringBuilder();
		sb.append("SELECT songview.idSong, strTitle, group_concat(DISTINCT strArtist) AS strArtists, strAlbum, " +
				"iTrack," +
				" iDuration, strPath, strFileName, art.url");
		...
		sb.append(" LEFT OUTER JOIN art ON art.media_id=songview.idSong AND art.media_type='song' AND art" +
				".type='thumb'");
		sb.append(" WHERE ");
		sb.append(sqlCondition);
	...
	new Song( // int id, String title, String artist, String album, int track, int duration, String path,
	String filename, String thumbPath
	Connection.trimInt(fields[row]),
	Connection.trim(fields[row + 1]),
	Connection.trim(fields[row + 2]),
	...
	private ArrayList<Song> parseSongs(String response) {
		...
		try {
			for (int row = 1; row < fields.length; row += 9) {
				songs.add(new Song( // int id, String title, String artist, String album, int track, int duration,
						// String path, String filename, String thumbPath
						Connection.trimInt(fields[row]),
						Connection.trim(fields[row + 1]),
						Connection.trim(fields[row + 2]),
						Connection.trim(fields[row + 3]),
