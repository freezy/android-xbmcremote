...
public class TvShowClient extends Client implements ITvShowClient {
	...
	public ArrayList<TvShow> getTvShows(INotifiableManager manager, int sortBy, String sortOrder,
		...
		sb.append("SELECT tvshow.idShow, tvshow.c00, \"\" AS c01, ROUND(tvshow.c04, 2), tvshow.c05, tvshow.c08, " +
				"tvshow.c13, tvshow.c14,");
		...
		sb.append("		SELECT min(tvshow.idShow) as idShow, tvshow.c00, count(1) AS totalcount, " +
...
/*		sb.append("SELECT tvshow.idShow, c00, c01, c04, c05, c08, c13, c14, strPath FROM tvshow, path,
tvshowlinkpath");
	...
	public ArrayList<Season> getSeasons(INotifiableManager manager, TvShow show, boolean hideWatched) {
		...
		sb.append(" LEFT OUTER JOIN art ON art.media_id=seasons.idSeason AND art.media_type='season' AND art" +
				".type='thumb'");
	...
	public ArrayList<Episode> getEpisodes(INotifiableManager manager, int sortBy, String sortOrder,
										  boolean hideWatched) {
		StringBuilder sb = new StringBuilder();
		sb.append("SELECT idEpisode, c00, \"\" AS c01, ROUND(c03, 2), c04, c05, c06, playCount, c10, c12, c13, " +
				"strPath, strFileName, strTitle, art.url");
		sb.append(" FROM episodeview LEFT OUTER JOIN art ON art.media_id=episodeview.idEpisode AND art" +
				".media_type='episode' AND art.type='thumb'");
		if (hideWatched) {
	 ...
	 * @return
	 */
	public ArrayList<Episode> getEpisodes(INotifiableManager manager, TvShow show, Season season, int sortBy,
	...
	public TvShow updateTvShowDetails(INotifiableManager manager, TvShow show) {
		...
		sb.append(" FROM actors LEFT JOIN art ON art.media_id=actors.idActor AND art.media_type='actor' AND art" +
				".type='thumb', actorlinktvshow");
	...
	public Episode updateEpisodeDetails(INotifiableManager manager, Episode episode) {
		...
		sb = new StringBuilder();
		sb.append("SELECT actors.idActor, strActor, art.url, strRole");
		sb.append(" FROM actors LEFT OUTER JOIN art ON art.media_id=actors.idActor AND art.media_type='actor' AND " +
				"art" +
				".type='thumb', actorlinkepisode");
	...
	private String showsOrderBy(int sortBy, String sortOrder) {
		switch (sortBy) {
			...
			case SortType.TITLE:
				return " ORDER BY CASE WHEN tvshow.c15 IS NULL OR tvshow.c15 = '' THEN replace(lower(tvshow.c00)," +
						"'the ','') ELSE replace(lower(tvshow.c15),'the ','') END " + sortOrder;
			case SortType.YEAR:
				return " ORDER BY tvshow.c05 " + sortOrder + ", CASE WHEN tvshow.c15 IS NULL OR tvshow.c15 = '' THEN" +
						" " +
						"lower(tvshow.c00) ELSE lower(tvshow.c15) END " + sortOrder;
