 ...
 * can subscribe. On any event, the {@link java.util.Observer#update(Observable o, Object arg) update()}
...
public class BroadcastListener extends Observable implements Runnable {
	...
	private void handle(String response) {
		...
		if (response.startsWith("StartUp")) {
			...
			sTimer.schedule(new BroadcastListener.Counter(currPlaying.getTime(), currPlaying.getDuration()), 0L,
					1000L);
