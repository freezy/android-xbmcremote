		...
		switch (mediaType) {
			...
			case MediaType.VIDEO_TVSHOW:
				...
				if (ar > 0.95 && ar < 1.05) { // square
					...
					return new Dimension((int) ((double) getPixel(size) / FULL_AR), getPixel(size),
							Dimension.LANDSCAPE);
				} else if (ar < 2) {            // landscape 16:9
					return new Dimension((int) ((double) getPixel(size) / LANDSCAPE_AR), getPixel(size),
							Dimension.LANDSCAPE);
				 ...
				 */
					switch (size) {
						case ThumbSize.SMALL:
							return new Dimension(getPixel(ThumbSize.SCREENWIDTH), (int) ((double) getPixel(ThumbSize
									.SCREENWIDTH) * BANNER_AR), Dimension.BANNER);
						case ThumbSize.MEDIUM:
							return new Dimension(getPixel(ThumbSize.SCREENHEIGHT), (int) ((double) getPixel(ThumbSize
									.SCREENHEIGHT) * BANNER_AR), Dimension.BANNER);
						default:
						case ThumbSize.BIG:
