...
public class EventClient implements IEventClient {
	...
	public EventClient(InetAddress hostAddress, int hostPort, String deviceName, byte iconType, byte[] iconData) {
		Log.i(TAG, "EventClient(" + hostAddress + ", " + hostPort + ", " + deviceName + ", " + iconType + ", " +
				"" + iconData + ")");
		mDeviceName = deviceName;
	 ...
	 * @param axis
	 */
	public void sendButton(String map_name, String button_name, boolean repeat, boolean down, boolean queue,
						   short amount, byte axis) {

		final InetAddress addr = mHostAddress;
		if (addr != null) {
			Log.i(TAG, "sendButton(" + map_name + ", \"" + button_name + "\", " + (repeat ? "rep, " : "nonrep, " +
					"") + (down ? "down)" : "up)"));
