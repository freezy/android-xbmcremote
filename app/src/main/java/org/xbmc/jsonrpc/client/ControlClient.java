...
public class ControlClient extends Client implements IControlClient {
	...
	public boolean addToPlaylist(INotifiableManager manager, String fileOrFolder, int playlistId) {
		...
		return mConnection.getString(manager, "Playlist.Add", obj().p("playlistid", playlistId).p("item",
				obj().p(type, fileOrFolder))).equals("OK");
	}

	public boolean play(INotifiableManager manager, int playlistId) {
		return mConnection.getString(manager, "Player.Open", obj().p("item", obj().p("playlistid", playlistId)))
				.equals("OK");
	}
	...
	public boolean playFile(INotifiableManager manager, String filename, int playlistId) {
		...
		return mConnection.getString(manager, "Player.Open", obj().p("item", obj().p("file", filename)).p("options",
				obj().p("resume", true))).equals("OK");
	}
	...
	public boolean playNext(INotifiableManager manager) {
		return mConnection.getString(manager, "Player.GoNext", obj().p("playlistid", getPlaylistId(manager))).equals
				("OK");
	}
	...
	public boolean playPrevious(INotifiableManager manager) {
		return mConnection.getString(manager, "Player.GoPrevious", obj().p("playlistid", getPlaylistId(manager)))
				.equals("OK");
	}
	...
	public boolean stop(INotifiableManager manager) {
		return mConnection.getString(manager, "Player.Stop", obj().p("playerid", getActivePlayerId(manager))).equals
				("OK");
	}
	...
	public boolean seek(INotifiableManager manager, SeekType type, int progress) {
		if (type.compareTo(SeekType.absolute) == 0)
			return mConnection.getJson(manager, "Player.Seek", obj().p("playerid",
					getActivePlayerId(manager)).p("value", progress)).get("percentage") != null;
		else
	...
	public int getPercentage(INotifiableManager manager) {
		ArrayNode arr = arr();
		arr.add("percentage");
		return mConnection.getInt(manager, "Player.GetProperties", obj().p("playerid", getActivePlayerId(manager)).p
				(PARAM_PROPERTIES, arr), "percentage");
	}
	...
	public int getVolume(INotifiableManager manager) {
		ArrayNode arr = arr();
		arr.add("volume");
		return mConnection.getInt(manager, "Application.GetProperties", obj().p("playerid", getActivePlayerId(manager)
		).p(PARAM_PROPERTIES, arr), "volume");
	}
	...
	public int getPlaylistId(INotifiableManager manager) {
		...
		arr.add("playlistid");
		return mConnection.getInt(manager, "Player.GetProperties", obj().p("playerid", getActivePlayerId(manager)).p
				(PARAM_PROPERTIES, arr), "playlistid");
	}
	...
	public boolean setPlaylistPos(INotifiableManager manager, int playlistId, int position) {
		...
		if (playerid == -1 || currentplaylistid != playlistId)
			return mConnection.getString(manager, "Player.Open", obj().p("item", obj().p("playlistid",
					playlistId).p("position", position))).equals("OK");
		else
	...
	public boolean setGuiSetting(INotifiableManager manager, final int setting, final String value) {
		return false;//mConnection.getBoolean(manager, "SetGUISetting", GuiSettings.getType(setting) + ";" +
	...
	public ICurrentlyPlaying getCurrentlyPlaying(INotifiableManager manager) {
		...
		arr.add("totaltime");
		arr.add("type");
		final JsonNode player_details = mConnection.getJson(manager, "Player.GetProperties", obj().p("playerid",
				playerid).p(PARAM_PROPERTIES, arr));
		...
		if (player_details != null) {
			...
			arr1.add("tagline");
			arr1.add("title");
			final JsonNode file_details = mConnection.getJson(manager, "Player.GetItem", obj().p("playerid",
					playerid).p(PARAM_PROPERTIES, arr1)).get("item");

			if (file_details.get("Filename") != null && file_details.get("Filename").getTextValue().contains
					("Nothing" +
					" " +
					"Playing")) {
				return nothingPlaying;
