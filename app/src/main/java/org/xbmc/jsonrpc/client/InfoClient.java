...
public class InfoClient extends Client implements IInfoClient {
	 ...
	 * @return
	 */
	public ArrayList<FileLocation> getDirectory(INotifiableManager manager, String path, DirectoryMask mask,
												int offset, int limit, final int mMediaType) {
		final ArrayList<FileLocation> dirs = new ArrayList<FileLocation>();
		final JsonNode jsonDirs = mConnection.getJson(manager, "Files.GetDirectory", sort(obj().p("media",
				"files").p("directory", path), SortType.ALBUM, "descending"), "files");
	...
	public ArrayList<FileLocation> getShares(INotifiableManager manager, int mediaType) {
		...
		final JsonNode jsonShares = mConnection.getJson(manager, "Files.GetSources", obj().p("media",
	 ...
	 */
	public String getCurrentlyPlayingThumbURI(INotifiableManager manager) throws MalformedURLException,
			URISyntaxException {
		...
		ArrayNode arr = arr();
		arr.add("thumbnail");
		final JsonNode item = mConnection.getJson(manager, "Player.GetItem", obj().p("playerid",
				playerid).p(PARAM_PROPERTIES, arr)).get("item");
	...
	public String getSystemInfo(INotifiableManager manager, int field) {
		ArrayNode arr = arr();
		arr.add("version");
		JsonNode version = mConnection.getJson(manager, "Application.GetProperties", obj().p(PARAM_PROPERTIES,
				arr)).get("version");
		return getInt(version, "major") + "." + getInt(version, "minor") + " " + getString(version,
	...
	public boolean getGuiSettingBool(INotifiableManager manager, int field) {
		//TODO
		//return mConnection.getBoolean(manager, "GetGuiSetting", GuiSettings.MusicLibrary.getType(field) + ";" +
	...
	public int getGuiSettingInt(INotifiableManager manager, int field) {
		//TODO
		//return mConnection.getInt(manager, "GetGuiSetting", GuiSettings.MusicLibrary.getType(field) + ";" +
		// GuiSettings.MusicLibrary.getName(field));
		return 0;
	}
	...
	public boolean setGuiSettingBool(INotifiableManager manager, int field, boolean value) {
		//TODO
		// return mConnection.getBoolean(manager, "SetGuiSetting", GuiSettings.getType(field) + ";" + GuiSettings
		// .getName(field) + ";" + value);
		return false;
	}
	...
	/**
	 * Returns any music info variable see {@link org.xbmc.api.info.MusicInfo}
	 *
	 * @param field Field to return
	 * @return
	 * @TODO Wait for JSON-RPC implementation
	...
	/**
	 * Returns any video info variable see {@link org.xbmc.api.info.VideoInfo}
	 *
	 * @param field Field to return
	 * @return
	 * @TODO Wait for JSON-RPC implementation
