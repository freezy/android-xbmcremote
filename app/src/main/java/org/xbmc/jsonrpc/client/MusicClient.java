...
public class MusicClient extends Client implements IMusicClient {
	...
	public boolean addToPlaylist(INotifiableManager manager, Album album, int sortBy, String sortOrder) {
		return mConnection.getString(manager, "Playlist.Add", obj().p("playlistid", PLAYLIST_ID).p("item",
				obj().p("albumid", album.id))).equals("OK");
	}
	...
	public boolean addToPlaylist(INotifiableManager manager, Artist artist, int sortBy, String sortOrder) {
		return mConnection.getString(manager, "Playlist.Add", obj().p("playlistid", PLAYLIST_ID).p("item",
				obj().p("artist", artist.id))).equals("OK");
	}
	...
	public boolean addToPlaylist(INotifiableManager manager, Genre genre, int sortBy, String sortOrder) {
		return mConnection.getString(manager, "Playlist.Add", obj().p("playlistid", PLAYLIST_ID).p("item",
				obj().p("genreid", genre.id))).equals("OK");
	}
	 ...
	 * @return True on success, false otherwise.
	 */
	public boolean addToPlaylist(INotifiableManager manager, Artist artist, Genre genre, int sortBy,
								 String sortOrder) {
		return mConnection.getString(manager, "Playlist.Add", obj().p("playlistid", PLAYLIST_ID).p("item",
				obj().p("genreid", genre.id).p("artistid", artist.id))).equals("OK");
	}
	...
	public boolean addToPlaylist(INotifiableManager manager, Song song) {
		return mConnection.getString(manager, "Playlist.Add", obj().p("playlistid", PLAYLIST_ID).p("item", obj().p
				("songid", song.id))).equals("OK");
	}
	...
	public int getPlaylistSize(INotifiableManager manager) {
		ArrayNode arr = arr();
		arr.add("size");
		return mConnection.getInt(manager, "Playlist.GetProperties", obj().p("playlistid", PLAYLIST_ID).p
				(PARAM_PROPERTIES, arr), "size");
	}
	...
	public int getPlaylistPosition(INotifiableManager manager) {
		ArrayNode arr = arr();
		arr.add("position");
		return mConnection.getInt(manager, "Player.GetItem", obj().p("playerid", getActivePlayerId(manager)).p
				(PARAM_PROPERTIES, arr), "position");
	}
	...
	public boolean setPlaylistPosition(INotifiableManager manager, int position) {
		...
		if (playerid == -1)
			return mConnection.getString(manager, "Player.Open", obj().p("item", obj().p("playlistid",
					PLAYLIST_ID).p("position", position))).equals("OK");
		else
			return mConnection.getString(manager, "Player.GoTo", obj().p("playerid",
					getActivePlayerId(manager)).p("position", position)).equals("OK");
	}
	...
	public boolean removeFromPlaylist(INotifiableManager manager, int position) {
		return mConnection.getString(manager, "Playlist.Remove", obj().p("playlistid", PLAYLIST_ID).p("position",
				position)).equals("OK");
	}
	...
	public boolean removeFromPlaylist(INotifiableManager manager, String path) {
		...
		arr.add("position");
		arr.add("file");
		JsonNode playlistitems = mConnection.getJson(manager, "Playlist.GetItems", obj().p("playlistid",
				PLAYLIST_ID).p(PARAM_PROPERTIES, arr));
		for (Iterator<JsonNode> i = playlistitems.getElements(); i.hasNext(); ) {
			...
			if (getString(jsonItem, "file").toLowerCase().equals(path.toLowerCase()))
				return mConnection.getString(manager, "Playlist.Remove", obj().p("playlistid",
						PLAYLIST_ID).p("position", getInt(jsonItem, "position"))).equals("OK");
		}
	...
	/**
	 * Returns the first songs of the playlist.
	 *
	...
	public ArrayList<String> getPlaylist(INotifiableManager manager) {
		ArrayNode arr = arr();
		arr.add("file");
		JsonNode jsonItems = mConnection.getJson(manager, "PlayList.GetItems", obj().p("playlistid",
				PLAYLIST_ID).p("limits", obj().p("start", 0).p("end", PLAYLIST_LIMIT)).p("properties", arr));
	...
	public boolean play(INotifiableManager manager) {
		return mConnection.getString(manager, "Player.Open", obj().p("item", obj().p("playlistid",
				PLAYLIST_ID).p("position", 0))).equals("OK");
	}
	...
	public boolean playNext(INotifiableManager manager) {
		return mConnection.getString(manager, "Player.GoNext", obj().p("playerid", getActivePlayerId(manager))).equals
				("OK");
	}
	...
	public boolean playPrev(INotifiableManager manager) {
		return mConnection.getString(manager, "Player.GoPrevious", obj().p("playerid", getActivePlayerId(manager)))
				.equals("OK");
	}
	...
	private ArrayList<Album> getAlbums(INotifiableManager manager, ObjNode obj, int sortBy, String sortOrder) {
		...
		arr.add("year");
		arr.add("thumbnail");
		obj = sort(obj.p(PARAM_PROPERTIES, arr), sortBy, sortOrder);
	...
	public ArrayList<Album> getAlbums(INotifiableManager manager, ArrayList<Integer> artistIDs) {
		...
		for (int id : artistIDs) {
			albums.addAll(getAlbums(manager, obj().p("filter", obj().p("artistid", id)), SortType.TITLE,
					"descending"));
		}
	...
	public ArrayList<Artist> getArtists(INotifiableManager manager, ObjNode obj, boolean albumArtistsOnly) {
		...
		ArrayNode arr = arr();
		arr.add("thumbnail");
		obj.p(PARAM_PROPERTIES, arr).p("albumartistsonly", albumArtistsOnly);
	...
	public ArrayList<Genre> getGenres(INotifiableManager manager) {
		...
		final JsonNode result = mConnection.getJson(manager, "AudioLibrary.GetGenres", sort(obj(), SortType.TITLE,
				"descending"));
	...
	public Album updateAlbumInfo(INotifiableManager manager, Album album) {
		...
		arr.add("albumlabel");
		arr.add("rating");
		final JsonNode result = mConnection.getJson(manager, "AudioLibrary.GetAlbumDetails", obj().p("albumid", album
				.id).p("properties", arr));
	...
	public Artist updateArtistInfo(INotifiableManager manager, Artist artist) {
		...
		arr.add("style");
		arr.add("description");
		final JsonNode result = mConnection.getJson(manager, "AudioLibrary.GetArtistDetails", obj().p("artistid",
				artist.id).p("properties", arr));
	...
	private ArrayList<Song> getSongs(INotifiableManager manager, ObjNode obj, int sortBy, String sortOrder) {
		...
		arr.add("file");
		arr.add("thumbnail");
		return getSongs(manager, sort(obj.p(PARAM_PROPERTIES, arr), sortBy, sortOrder));
	}
	 ...
	 * @return All tracks of the genre
	 */
	public ArrayList<Song> getSongs(INotifiableManager manager, Artist artist, Genre genre, int sortBy,
									String sortOrder) {
		return getSongs(manager, obj().p("filter", obj().p("artistid", artist.id).p("genreid", genre.id)), sortBy,
				sortOrder);
	}
	...
	public Bitmap getCover(INotifiableManager manager, ICoverArt cover, int size) {
		...
		if (Album.getThumbUri(cover) != "") {
			final JsonNode dl = mConnection.getJson(manager, "Files.PrepareDownload", obj().p("path", Album
					.getThumbUri(cover)));
			if (dl != null) {
	...
	public ArrayList<Integer> getCompilationArtistIDs(INotifiableManager manager) {
		...
		for (Artist artist : artists) {
			if (artist.name.toLowerCase().equals("various artists") || artist.name.toLowerCase().equals("v.a.") ||
					artist.name.toLowerCase().equals("va"))
				ids.add(artist.id);
		}
