...
public class VideoClient extends Client implements IVideoClient {
	...
	public ArrayList<Movie> getMovies(INotifiableManager manager, ObjNode obj, int sortBy, String sortOrder,
									  boolean hideWatched) {
	...
	public Movie updateMovieDetails(INotifiableManager manager, Movie movie) {
		...
		arr.add("trailer");
		arr.add("votes");
		ObjNode obj = obj().p("movieid", movie.getId()).p(PARAM_PROPERTIES, arr);
	...
	public Bitmap getCover(INotifiableManager manager, ICoverArt cover, int size) {
		...
		if (Movie.getThumbUri(cover) != "") {
			final JsonNode dl = mConnection.getJson(manager, "Files.PrepareDownload", obj().p("path",
					Movie.getThumbUri(cover)));
			if (dl != null) {
	...
	public int getPlaylistPosition(INotifiableManager manager) {
		...
		ArrayNode arr = arr();
		arr.add("position");
		return mConnection.getInt(manager, "Player.GetProperties", obj().p("playerid", getInt(active,
				"playerid")).p(PARAM_PROPERTIES, arr), "position");
	}
