...
class DiskCacheThread extends AbstractThread {
	...
	public static Bitmap getCover(ICoverArt cover, int thumbSize) {
		final File file = ImportUtilities.getCacheFile(MediaType.getArtFolder(cover.getMediaType()), thumbSize,
	...
	public static boolean isInCache(ICoverArt cover, int thumbSize) {
		final File file = ImportUtilities.getCacheFile(MediaType.getArtFolder(cover.getMediaType()), thumbSize,
				Crc32.formatAsHexLowerCase(cover.getCrc()).toLowerCase());
	...
	public void getCover(final DataResponse<Bitmap> response, final ICoverArt cover, final int thumbSize,
		...
		mHandler.post(new Runnable() {
			public void run() {
				if (cover != null) {
					final File file = ImportUtilities.getCacheFile(MediaType.getArtFolder(cover.getMediaType()),
							thumbSize, Crc32.formatAsHexLowerCase(cover.getCrc()));
					if (file.exists()) {
						final Bitmap bitmap = BitmapFactory.decodeFile(file.getAbsolutePath());
