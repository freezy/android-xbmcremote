...
public class FiveLabelsItemView extends AbstractItemView {
	...
	protected void drawPortraitText(Canvas canvas) {
		...
		PAINT.setFakeBoldText(false);
		if (subtitle != null) {
			canvas.drawText(ellipse(subtitle, width - subtitleRightWidth - size50 - (3 * padding)), size55, size42,
					PAINT);
		}
		if (bottomtitle != null) {
			canvas.drawText(ellipse(bottomtitle, width - bottomrightWidth - size50 - (3 * padding)), size55, size59,
					PAINT);
		}
	}
