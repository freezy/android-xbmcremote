...
public class NowPlayingController extends AbstractController implements INotifiableController, IController, Callback {
	...
	public synchronized boolean handleMessage(Message msg) {
		...
		switch (msg.what) {
			...
			case NowPlayingPollerThread.MESSAGE_PLAYLIST_ITEM_CHANGED:
				mNowPlayingActivity.updateInfo(currentlyPlaying.getTitle(), currentlyPlaying.getArtist(),
			...
			case NowPlayingPollerThread.MESSAGE_COVER_CHANGED:
				...
				mNowPlayingActivity.updateCover(ConnectionFactory.getNowPlayingPoller(mActivity).getNowPlayingCover(),
						(currentlyPlaying != null) ? currentlyPlaying.getMediaType() : MediaType.UNKNOWN);
				return true;
			...
			public void onClick(View v) {
				switch (mPlayStatus) {
					case PlayStatus.PLAYING:
						mEventClientManager.sendButton("R1", ButtonCodes.REMOTE_PAUSE, false, true, true, (short) 0,
								(byte) 0);
						break;
					case PlayStatus.PAUSED:
						mEventClientManager.sendButton("R1", ButtonCodes.REMOTE_PLAY, false, true, true, (short) 0,
								(byte) 0);
						break;
	...
	public void onActivityResume(final Activity activity) {
		...
		new Thread("nowplaying-spawning") {
			@Override
			public void run() {
				ConnectionFactory.subscribeNowPlayingPollerThread(activity.getApplicationContext(),
						mNowPlayingHandler);
			}
		}.start();
		if (mControlManager != null) {
