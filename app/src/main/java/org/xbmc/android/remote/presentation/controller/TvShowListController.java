...
public class TvShowListController extends ListController implements IController {
	...
	private void fetch() {
		...
		final String title = mActor != null ? mActor.name + " - " : mGenre != null ? mGenre.name + " - " : "" + "TV " +
				"Shows";
	...
	public void onContextItemSelected(MenuItem item) {
		final TvShow show = (TvShow) mList.getAdapter().getItem(((FiveLabelsItemView) ((AdapterContextMenuInfo) item
		...
		switch (item.getItemId()) {
			...
			case ITEM_CONTEXT_IMDB:
				...
				if (mActivity.getPackageManager().resolveActivity(intentIMDb, PackageManager.MATCH_DEFAULT_ONLY) ==
						null) {
					intentIMDb = new Intent(Intent.ACTION_VIEW, Uri.parse("http://www.imdb.com/search/title?title=" +
							show.getShortName() + "&title_type=tv_series&release_date=" + show.firstAired));
				}
