...
public class MovieListController extends ListController implements IController {
	...
	private void fetch() {
		final String title = mActor != null ? mActor.name + " - " : mGenre != null ? mGenre.name + " - " : "" +
				"Movies";
	...
	public void onContextItemSelected(MenuItem item) {
		final Movie movie = (Movie) mList.getAdapter().getItem(((FiveLabelsItemView) ((AdapterContextMenuInfo) item
		...
		switch (item.getItemId()) {
			...
			case ITEM_CONTEXT_IMDB:
				...
				if (mActivity.getPackageManager().resolveActivity(intentIMDb, PackageManager.MATCH_DEFAULT_ONLY) ==
						null) {
					if (movie.getIMDbId().equals("")) {
						intentIMDb = new Intent(Intent.ACTION_VIEW, Uri.parse("http://www.imdb" +
								".com/search/title?title=" + movie.getShortName() + "&title_type=feature," +
								"tv_movie&release_date=" + movie.year));
					} else {
						intentIMDb = new Intent(Intent.ACTION_VIEW, Uri.parse("http://www.imdb.com/title/" + movie
								.getIMDbId()));
					}
				}
...
//		menu.add(0, MENU_SWITCH_VIEW, 0, "Switch view").setIcon(R.drawable.menu_view);
	...
	public void onOptionsItemSelected(MenuItem item) {
		...
		switch (item.getItemId()) {
			...
			case MENU_SORT_BY_TITLE_ASC:
				...
				ed.putString(AbstractManager.PREF_SORT_ORDER_PREFIX + AbstractManager.PREF_SORT_KEY_MOVIE,
						SortType.ORDER_ASC);
				ed.commit();
				fetch();
				break;
			...
			case MENU_SORT_BY_DATE_ADDED_ASC:
				ed = mActivity.getPreferences(Context.MODE_PRIVATE).edit();
				ed.putInt(AbstractManager.PREF_SORT_BY_PREFIX + AbstractManager.PREF_SORT_KEY_MOVIE,
						SortType.DATE_ADDED);
				ed.putString(AbstractManager.PREF_SORT_ORDER_PREFIX + AbstractManager.PREF_SORT_KEY_MOVIE,
						SortType.ORDER_ASC);
				ed.commit();
	...
	private class MovieAdapter extends ArrayAdapter<Movie> {
		...
		public View getView(int position, View convertView, ViewGroup parent) {
			...
			if (convertView == null) {
				view = new FiveLabelsItemView(mActivity, mVideoManager, parent.getWidth(), mFallbackBitmap,
