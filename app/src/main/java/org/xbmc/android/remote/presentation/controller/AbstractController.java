					...
					public void onClick(DialogInterface dialog, int which) {
						...
						(new Thread() {
							public void run() {
								...
								int wait = 0;
								while (wait <= MAX_WAIT_FOR_WIFI * 1000 && helper.getWifiState() != WifiHelper
										.WIFI_STATE_ENABLED) {
									try {
										sleep(500);
										wait += 500;
						...
						pd.setOnDismissListener(new OnDismissListener() {
							public void onDismiss(DialogInterface dialog) {
								if (helper.getWifiState() != WifiHelper.WIFI_STATE_CONNECTED) {
									...
									builder.setMessage("You can open the Wifi settings or wait " + MAX_WAIT_FOR_WIFI +
											" seconds");
									...
									builder.setNegativeButton("Wait", new OnClickListener() {
										public void onClick(DialogInterface dialog, int which) {
											mDialogShowing = false;
											mActivity.runOnUiThread(mWaitForWifi); //had to make the Thread a field
...
//					intent.putExtra(SettingsActivity.JUMP_TO, SettingsActivity.JUMP_TO_INSTANCES);
			...
			builder.setMessage("Wrong data from HTTP API; expected '" + e.getExpected() + "', " +
					"got '" + e.getReceived() + "'.");
			...
			if (e.getMessage() != null && e.getMessage().startsWith("Network unreachable")) {
				builder.setTitle("No network");
				builder.setMessage("XBMC Remote needs local network access. Please make sure that your wireless " +
						"network is activated. You can click on the Settings button below to directly access your " +
						"network settings.");
				builder.setNeutralButton("Settings", new OnClickListener() {
					public void onClick(DialogInterface dialog, int which) {
						mActivity.startActivity(new Intent(android.provider.Settings.ACTION_WIRELESS_SETTINGS));
