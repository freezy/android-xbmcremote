...
public class SettingsController extends AbstractController implements INotifiableController, IController,
		...
		for (String key : ps.getSharedPreferences().getAll().keySet()) {
			...
			if (pref != null && pref.getSummary() != null) {
				...
				if (summary.contains(SettingsActivity.SUMMARY_VALUE_PLACEHOLDER)) {
					pref.setSummary(summary.replaceAll(SettingsActivity.SUMMARY_VALUE_PLACEHOLDER,
							ps.getSharedPreferences().getString(key, "<not set>")));
				}
			}
		}
	}
	...
	public void onCreateOptionsMenu(Menu menu) {
		...
		menu.addSubMenu(0, MENU_ADD_FROM_BARCODE, 0, mPreferenceActivity.getString(R.string.add_from_barcode)).setIcon
				(R.drawable.menu_qr_code);
		menu.addSubMenu(0, MENU_GENERATE_BARCODE, 0, mPreferenceActivity.getString(R.string.generate_barcode)).setIcon
				(R.drawable.menu_qr_code);
	}
		...
		switch (item.getItemId()) {
			...
			case MENU_GENERATE_BARCODE:
				...
				if (hosts.size() == 0) {
					...
					builder.setPositiveButton(mPreferenceActivity.getString(R.string.ok),
		...
		if (requestCode == REQUEST_SCAN_BARCODE) {
			if (resultCode == Activity.RESULT_OK) {
				...
				if (host == null) {
					...
					Toast.makeText(mPreferenceActivity, mPreferenceActivity.getString(R.string.added_host, host.name),
							Toast.LENGTH_SHORT).show();

					// Is there a better way to refresh the preferences screen?
					mPreferenceActivity.startActivity(new Intent(mPreferenceActivity.getBaseContext(),
							HostSettingsActivity.class));
