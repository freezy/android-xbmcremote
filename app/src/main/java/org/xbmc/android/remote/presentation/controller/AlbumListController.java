...
public class AlbumListController extends ListController implements IController {
	...
	private void fetch() {
		final String title = mArtist != null ? mArtist.name + " - " : mGenre != null ? mGenre.name + " - " : "" +
	...
	public void onContextItemSelected(MenuItem item) {
		final Album album = (Album) mList.getAdapter().getItem(((ThreeLabelsItemView) ((AdapterContextMenuInfo) item
		...
		switch (item.getItemId()) {
			...
			case ITEM_CONTEXT_INFO:
				DialogFactory.getAlbumDetail(mMusicManager, mActivity, album, mActivity.getApplicationContext())
						.show();
				break;
			default:
				return;
	...
	public void onOptionsItemSelected(MenuItem item) {
		...
		switch (item.getItemId()) {
			...
			case MENU_SORT_BY_ALBUM_ASC:
				...
				ed.putString(AbstractManager.PREF_SORT_ORDER_PREFIX + AbstractManager.PREF_SORT_KEY_ALBUM,
						SortType.ORDER_ASC);
				ed.commit();
				fetch();
				break;
