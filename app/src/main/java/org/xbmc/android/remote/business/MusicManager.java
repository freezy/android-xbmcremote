...
public class MusicManager extends AbstractManager implements IMusicManager, ISortableManager, INotifiableManager {
	...
	public void getAlbums(final DataResponse<ArrayList<Album>> response, final Context context) {
		mHandler.post(new Command<ArrayList<Album>>(response, this) {
			@Override
			public void doRun() throws Exception {
				response.value = music(context).getAlbums(MusicManager.this, getSortBy(SortType.ALBUM),
						getSortOrder());
			}
		});
	}
...
//			public void run() { 
//				response.value = music(context).getAlbums(MusicManager.this, artist, getSortBy(SortType.ALBUM),
// getSortOrder());
//				onFinish(response);
//			}
//		});
	 ...
	 *
	 * @param response Response object
	 * @param genre    Genre of the albums
	 */
	public void getAlbums(final DataResponse<ArrayList<Album>> response, final Genre genre, final Context context) {
//		mHandler.post(new Runnable() {
//			public void run() { 
//				response.value = music(context).getAlbums(MusicManager.this, genre, getSortBy(SortType.ALBUM),
// getSortOrder());
//				onFinish(response);
//			}
//		});
...
//		mHandler.post(new Runnable() {
//			public void run() { 
//				response.value = music(context).getSongs(MusicManager.this, album, getSortBy(SortType.ARTIST),
// getSortOrder());
//				onFinish(response);
//			}
//		});
		mHandler.post(new Command<ArrayList<Song>>(response, this) {
			public void doRun() throws Exception {
				response.value = music(context).getSongs(MusicManager.this, album, getSortBy(SortType.TRACK),
						getSortOrder());
			}
		});
	}
...
//		mHandler.post(new Runnable() {
//			public void run() { 
//				response.value = music(context).getSongs(MusicManager.this, artist, getSortBy(SortType.ARTIST),
// getSortOrder());
//				onFinish(response);
//			}
//		});
...
//		mHandler.post(new Runnable() {
//			public void run() { 
//				response.value = music(context).getSongs(MusicManager.this, genre, getSortBy(SortType.ARTIST),
// getSortOrder());
//				onFinish(response);
//			}
//		});
...
//				try {
//					albumArtistsOnly = info(context).getGuiSettingBool(MusicManager.this,
// GuiSettings.MusicLibrary.ALBUM_ARTISTS_ONLY);
...
//		});
		mHandler.post(new Command<ArrayList<Artist>>(response, this) {
			public void doRun() throws Exception {
				final boolean albumArtistsOnly = !info(context).getGuiSettingBool(MusicManager.this,
...
//		});
	...
	public void addToPlaylist(final DataResponse<Boolean> response, final Genre genre, final Context context) {
		mHandler.post(new Command<Boolean>(response, this) {
			public void doRun() throws Exception {
				...
				final IControlClient cc = control(context);
				final int numAlreadyQueued = mc.getPlaylistSize(MusicManager.this);
				response.value = mc.addToPlaylist(MusicManager.this, genre, getSortBy(SortType.ARTIST),
						getSortOrder());
				checkForPlayAfterQueue(mc, cc, numAlreadyQueued);
			}
		});
		...
		mHandler.post(new Command<Boolean>(response, this) {
			public void doRun() throws Exception {
				...
				if (playlistSize == 0) {  // if playlist is empty, add the whole album
					int n = 0;
					for (Song albumSong : mc.getSongs(MusicManager.this, album, getSortBy(PREF_SORT_KEY_ALBUM),
							getSortOrder())) {
	...
	public void addToPlaylist(final DataResponse<Boolean> response, final Artist artist, final Context context) {
		mHandler.post(new Command<Boolean>(response, this) {
			public void doRun() throws Exception {
				...
				final IControlClient cc = control(context);
				final int numAlreadyQueued = mc.getPlaylistSize(MusicManager.this);
				response.value = mc.addToPlaylist(MusicManager.this, artist, getSortBy(SortType.ALBUM),
						getSortOrder());
				checkForPlayAfterQueue(mc, cc, numAlreadyQueued);
			}
		});
	...
	public void addToPlaylist(final DataResponse<Boolean> response, final Artist artist, final Genre genre,
		...
		mHandler.post(new Command<Boolean>(response, this) {
			public void doRun() throws Exception {
				...
				final IControlClient cc = control(context);
				final int numAlreadyQueued = mc.getPlaylistSize(MusicManager.this);
				response.value = mc.addToPlaylist(MusicManager.this, artist, genre, getSortBy(SortType.ARTIST),
						getSortOrder());
				checkForPlayAfterQueue(mc, cc, numAlreadyQueued);
			}
		});
	...
	public void play(final DataResponse<Boolean> response, final Album album, final Context context) {
		mHandler.post(new Command<Boolean>(response, this) {
			public void doRun() throws Exception {
				...
				response.value = music(context).play(MusicManager.this, album, getSortBy(SortType.TRACK),
						getSortOrder());
			}
		});
	}
	...
	public void play(final DataResponse<Boolean> response, final Genre genre, final Context context) {
		mHandler.post(new Command<Boolean>(response, this) {
			public void doRun() throws Exception {
				control(context).stop(MusicManager.this);
				response.value = music(context).play(MusicManager.this, genre, getSortBy(SortType.ARTIST),
						getSortOrder());
			}
		});
	}
	...
	public void play(final DataResponse<Boolean> response, final Album album, final Song song, final Context context) {
		mHandler.post(new Command<Boolean>(response, this) {
			public void doRun() throws Exception {
				...
				for (Song albumSong : mc.getSongs(MusicManager.this, album, getSortBy(SortType.TRACK),
						getSortOrder())) {
					if (albumSong.id == song.id) {
						playPos = n;
						break;
	...
	public void play(final DataResponse<Boolean> response, final Artist artist, final Context context) {
		mHandler.post(new Command<Boolean>(response, this) {
			public void doRun() throws Exception {
				control(context).stop(MusicManager.this);
				response.value = music(context).play(MusicManager.this, artist, getSortBy(SortType.ALBUM),
						getSortOrder());
			}
		});
	}
