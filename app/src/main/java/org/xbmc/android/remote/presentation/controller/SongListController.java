...
public class SongListController extends ListController implements IController {
		...
		if (!isCreated()) {
			...
			mList.setOnItemClickListener(new OnItemClickListener() {
				public void onItemClick(AdapterView<?> parent, View view, int position, long id) {
					...
					final int SelectionType = Integer.parseInt(prefs.getString(PREF_DEFAULT_SELECTION_ACTION,
							DEFAULT_ACTION_PLAY));
					switch (SelectionType) {
						case INT_DEFAULT_ACTION_PLAY:
							if (mAlbum == null) {
								...
								mMusicManager.play(new QueryResponse(
										mActivity,
										"Playing album \"" + song.album + "\" starting with song \"" + song.title +
												"\" by " + song.artist + "...",
										"Error playing song!",
										true
						...
						case INT_DEFAULT_ACTION_QUEUE:
							if (mAlbum == null) {
								mMusicManager.addToPlaylist(new QueryResponse(mActivity, "Song added to playlist.",
										"Error adding song!"), song, mActivity.getApplicationContext());
							} else {
								mMusicManager.addToPlaylist(new QueryResponse(mActivity, "Playlist empty, " +
												"added whole album.", "Song added to playlist."), mAlbum, song,
	...
	private void fetch() {
		final String title = mAlbum != null ? mAlbum.name + " - " : mArtist != null ? mArtist.name + " - " : mGenre !=
				null ? mGenre.name + " - " : "" + "Songs";
	...
	public void onContextItemSelected(MenuItem item) {
		// be aware that this must be explicitly called by your activity!
		final Song song = (Song) mList.getAdapter().getItem(((ThreeLabelsItemView) ((AdapterContextMenuInfo) item
		...
		switch (item.getItemId()) {
			case ITEM_CONTEXT_QUEUE:
				if (mAlbum == null) {
					...
					mMusicManager.addToPlaylist(new QueryResponse(mActivity, "Playlist empty, added whole album.",
							"Song added to playlist."), mAlbum, song, mActivity.getApplicationContext());
				}
				break;
			case ITEM_CONTEXT_PLAY:
				if (mAlbum == null) {
					...
					mMusicManager.play(new QueryResponse(
							mActivity,
							"Playing album \"" + song.album + "\" starting with song \"" + song.title + "\" by " +
									song.artist + "...",
							"Error playing song!",
							true
					), mAlbum, song, mActivity.getApplicationContext());
	...
	public void onOptionsItemSelected(MenuItem item) {
		...
		switch (item.getItemId()) {
			...
			case MENU_SORT_BY_ALBUM_ASC:
				...
				ed.putString(AbstractManager.PREF_SORT_ORDER_PREFIX + AbstractManager.PREF_SORT_KEY_SONG,
						SortType.ORDER_ASC);
				ed.commit();
				fetch();
				break;
