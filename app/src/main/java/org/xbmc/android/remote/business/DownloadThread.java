...
class DownloadThread extends AbstractThread {
	 ...
	 */
	public static boolean download(final DataResponse<Bitmap> response, final ICoverArt cover, final int thumbSize,
								   final INotifiableController controller, final INotifiableManager manager,
								   final Context context, final boolean addToMemCache) {
		...
		if (bitmap != null) {
			...
			if (addToMemCache) {
				...
				if (DEBUG)
					Log.i(TAG, "Adding null-value (" + cover.getCrc() + ") to mem cache in order to block future " +
							"downloads");
		...
		mHandler.post(new Runnable() {
			public void run() {
				if (cover != null) {
					 ...
					 */
					if (thumbSize < ThumbSize.BIG && MemCacheThread.isInCache(cover,
							thumbSize)) { // we're optimistic, let's check the memory first.
