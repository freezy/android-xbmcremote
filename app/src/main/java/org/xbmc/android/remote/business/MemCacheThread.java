...
class MemCacheThread extends AbstractThread {
	...
	private static final HashMap<Long, SoftReference<Bitmap>> sCacheMedium = new HashMap<Long,
	...
	public static void addCoverToCache(ICoverArt cover, Bitmap bitmap, int thumbSize) {
		// if bitmap is null, add an entry to the sNotAvailable table so we can return the default bitmap later
		// directly.
		if (bitmap == null) {
	...
	public void getCover(final DataResponse<Bitmap> response, final ICoverArt cover, final int thumbSize,
						 final INotifiableController controller, final Bitmap defaultCover) {
		if (controller == null) {
