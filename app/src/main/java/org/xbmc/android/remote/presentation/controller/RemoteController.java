...
//						Log.i("RemoteController", "Saving previous value " + GuiSettings.getName(GuiSettings.Services
// .EVENT_SERVER_INITIAL_DELAY) + " = " + value);
//					}
//				});
//			}
	...
	public IGestureListener startGestureThread(final Context context) {
		...
		IGestureListener listener = new IGestureListener() {
			...
			private void scroll(String button, double amount) {
				if (amount != 0) {
					if (!mScrolling) {
						Log.i(TAG, "Setting " + GuiSettings.getName(GuiSettings.Services.EVENT_SERVER_INITIAL_DELAY) +
								" = " + 25);
						mInfoManager.setGuiSettingInt(new DataResponse<Boolean>(),
								GuiSettings.Services.EVENT_SERVER_INITIAL_DELAY, 25, context);
					}
					mEventClientManager.sendButton("XG", button, true, true, false, (short) (amount * 65535),
							(byte) 0);
					mScrolling = true;
				} else {
					mEventClientManager.sendButton("XG", button, false, false, false, (short) 0, (byte) 0);
					Log.i(TAG, "Restoring " + GuiSettings.getName(GuiSettings.Services.EVENT_SERVER_INITIAL_DELAY) +
							"" +
							" = " + mEventServerInitialDelay);
					mInfoManager.setGuiSettingInt(new DataResponse<Boolean>(),
							GuiSettings.Services.EVENT_SERVER_INITIAL_DELAY, mEventServerInitialDelay, context);
					mScrolling = false;
				}
			}
			...
			public void onSelect() {
				mEventClientManager.sendButton("R1", ButtonCodes.REMOTE_SELECT, false, true, false, (short) 0,
						(byte) 0);
			}

			public void onScrollDown() {
				Log.d(TAG, "onScrollDown()");
				mEventClientManager.sendButton("KB", ButtonCodes.KEYBOARD_PAGEDOWN, false, true, true, (short) 0,
						(byte) 0);
			}

			public void onScrollUp() {
				Log.d(TAG, "onScrollUp()");
				mEventClientManager.sendButton("KB", ButtonCodes.KEYBOARD_PAGEUP, false, true, true, (short) 0,
						(byte) 0);
			}
	...
	public boolean onKeyDown(int keyCode, KeyEvent event) {
		...
		switch (keyCode) {
			case KeyEvent.KEYCODE_VOLUME_UP:
				mEventClientManager.sendButton("R1", ButtonCodes.REMOTE_VOLUME_PLUS, false, true, true, (short) 0,
						(byte) 0);
				return true;
			case KeyEvent.KEYCODE_VOLUME_DOWN:
				mEventClientManager.sendButton("R1", ButtonCodes.REMOTE_VOLUME_MINUS, false, true, true, (short) 0,
						(byte) 0);
				return true;
	...
	private boolean onDirectionalPadDown(int keyCode) {
		...
		if (newstamp - mTimestamp > DPAD_DOWN_MIN_DELTA_TIME) {
			...
			switch (keyCode) {
				case KeyEvent.KEYCODE_DPAD_DOWN:
					mEventClientManager.sendButton("R1", ButtonCodes.REMOTE_DOWN, false, true, true, (short) 0,
							(byte) 0);
					return true;
				case KeyEvent.KEYCODE_DPAD_UP:
					mEventClientManager.sendButton("R1", ButtonCodes.REMOTE_UP, false, true, true, (short) 0,
							(byte) 0);
					return true;
				case KeyEvent.KEYCODE_DPAD_LEFT:
					mEventClientManager.sendButton("R1", ButtonCodes.REMOTE_LEFT, false, true, true, (short) 0,
							(byte) 0);
					return true;
				case KeyEvent.KEYCODE_DPAD_RIGHT:
					mEventClientManager.sendButton("R1", ButtonCodes.REMOTE_RIGHT, false, true, true, (short) 0,
							(byte) 0);
					return true;
				case KeyEvent.KEYCODE_DPAD_CENTER:
					mEventClientManager.sendButton("R1", ButtonCodes.REMOTE_ENTER, false, true, true, (short) 0,
							(byte) 0);
					return true;
				default:
					return false;
	 ...
	 *
	 * @param btn     Resource ID of the button
		...
		public void run() {
			...
			while (!mQuit) {
				try {
					switch (mAction) {
						case ACTION_UP:
							mEventClient.sendButton("R1", ButtonCodes.REMOTE_UP, false, true, true, (short) 0,
									(byte) 0);
							break;
						case ACTION_RIGHT:
							mEventClient.sendButton("R1", ButtonCodes.REMOTE_RIGHT, false, true, true, (short) 0,
									(byte) 0);
							break;
						case ACTION_DOWN:
							mEventClient.sendButton("R1", ButtonCodes.REMOTE_DOWN, false, true, true, (short) 0,
									(byte) 0);
							break;
						case ACTION_LEFT:
							mEventClient.sendButton("R1", ButtonCodes.REMOTE_LEFT, false, true, true, (short) 0,
									(byte) 0);
							break;
					}
	...
	private class OnRemoteAction implements OnTouchListener {
		...
		public boolean onTouch(View v, MotionEvent event) {
			if (event.getAction() == MotionEvent.ACTION_DOWN) {
				...
				mEventClientManager.sendButton("R1", mAction, !prefs.getBoolean("setting_send_repeats", false), true,
						true, (short) 0, (byte) 0);
				currKeyAction = mAction;

				if (prefs.getBoolean("setting_send_repeats", false) && !prefs.getBoolean("setting_send_single_click",
						false)) {
