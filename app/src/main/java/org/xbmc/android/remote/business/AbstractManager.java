...
public abstract class AbstractManager implements INotifiableManager {
	...
	public void getCover(final DataResponse<Bitmap> response, final ICoverArt cover, final int thumbSize,
						 final Bitmap defaultCover, final Context context, final boolean getFromCacheOnly) {
			...
			public void run() {
				if (cover.getCrc() != 0L) {
					...
					if (thumbSize == ThumbSize.SMALL || thumbSize == ThumbSize.MEDIUM) {
						if (DEBUG)
							Log.i(TAG, "[" + cover.getId() + ThumbSize.getDir(thumbSize) + "] Trying memory (" + Crc32
									.formatAsHexLowerCase(cover.getCrc()) + ")");
						getCoverFromMem(response, cover, thumbSize, defaultCover, context, getFromCacheOnly);
					} else {
						if (getFromCacheOnly) {
							Log.e(TAG, "[" + cover.getId() + ThumbSize.getDir(thumbSize) + "] ERROR: NOT downloading" +
									" " +
									"big covers is a bad idea because they are not cached!");
							response.value = null;
							onFinish(response);
						} else {
							if (DEBUG)
								Log.i(TAG, "[" + cover.getId() + ThumbSize.getDir(thumbSize) + "] Downloading " +
										"directly");
	...
	protected void getCoverFromMem(final DataResponse<Bitmap> response, final ICoverArt cover, final int thumbSize,
								   Bitmap defaultCover, final Context context, final boolean getFromCacheOnly) {
	 ...
	 * @param thumbSize Cover size
	 */
	protected void getCoverFromNetwork(final DataResponse<Bitmap> response, final ICoverArt cover,
									   final int thumbSize, final Context context) {
