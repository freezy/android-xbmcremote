...
public class FlexibleItemView extends FiveLabelsItemView {
	...
	private void drawLandscape(Canvas canvas, int drawWidth) {
		...
		PAINT.setFakeBoldText(false);
		if (subtitle != null) {
			canvas.drawText(ellipse(subtitle, width - (int) subtitleRightWidth - size50), drawWidth + padding, size42,
					PAINT);
		}
	...
	public void setCover(Bitmap cover) {
		mCover = cover;
		if (mCover != null) {
			Dimension renderDim = ThumbSize.getTargetDimension(ThumbSize.SMALL, MediaType.VIDEO, mCover.getWidth(),
					mCover.getHeight());
