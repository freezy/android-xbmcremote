...
public class AndroidBroadcastReceiver extends BroadcastReceiver {
	...
	public void onReceive(Context context, Intent intent) {
		...
		if (eventClient != null) {
			try {
				if (action.equals(android.telephony.TelephonyManager.ACTION_PHONE_STATE_CHANGED) && prefs.getBoolean
						("setting_show_call", false)) {
					...
					if (extra.equals(android.telephony.TelephonyManager.EXTRA_STATE_RINGING)) {
						...
						Bitmap pic;
						if (id != null)
							pic = Contacts.People.loadContactPhoto(context, Uri.withAppendedPath(Contacts.People
									.CONTENT_URI, id), R.drawable.icon, null);
						else
						...
						cm.getCurrentlyPlaying(new DataResponse<ICurrentlyPlaying>() {
							public void run() {
								if (value != null && value.isPlaying()) {
									eventClient.sendButton("R1", ButtonCodes.REMOTE_PAUSE, false, true, true,
											(short) 0, (byte) 0);
									sPlayState = PLAY_STATE_PAUSED;
								}
							}
						}, null);
						eventClient.sendNotification(callername, "calling", Packet.ICON_PNG,
								pic == null ? null : os.toByteArray());
						...
						if (sPlayState == PLAY_STATE_PAUSED) {
							eventClient.sendButton("R1", ButtonCodes.REMOTE_PLAY, true, true, true, (short) 0,
									(byte) 0);
							eventClient.sendButton("R1", ButtonCodes.REMOTE_PLAY, false, false, true, (short) 0,
									(byte) 0);
						}
					...
					if (eventClient != null) {
						...
						if (bundle != null) {
							...
							if (pic != null) {
								ByteArrayOutputStream os = new ByteArrayOutputStream();
								pic.compress(Bitmap.CompressFormat.PNG, 0, os);
								eventClient.sendNotification("SMS Received from " + msg.getContactName(),
										msg.getMessageBody(), Packet.ICON_PNG, os.toByteArray());
							} else
								eventClient.sendNotification("SMS Received from " + msg.getContactName(),
										msg.getMessageBody());
						}
					}
				} else if (action.equals(Intent.ACTION_SCREEN_OFF) && prefs.getBoolean("setting_show_notification",
						false)) {
					NowPlayingNotificationManager.getInstance(context).stopNotificating();
				} else if (action.equals(Intent.ACTION_SCREEN_ON) && prefs.getBoolean("setting_show_notification",
						false)) {
