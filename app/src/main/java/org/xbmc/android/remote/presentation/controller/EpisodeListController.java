		...
		switch (item.getItemId()) {
			...
			case ITEM_CONTEXT_PLAY_FROM_HERE:
				...
				for (int i = 0; i < numitems; i++)
					mControlManager.addToPlaylist(new DataResponse<Boolean>(), ((Episode) mList.getAdapter().getItem
							(i)).fileName, 1, mActivity);
			...
			case ITEM_CONTEXT_QUEUE:
				mControlManager.addToPlaylist(new QueryResponse(mActivity, "Queued episode: " + episode.getName(),
						"Error queueing file."), episode.fileName, 1, mActivity);
				break;
			default:
				return;
		}
	...
	public void onOptionsItemSelected(MenuItem item) {
		...
		switch (item.getItemId()) {
			...
			case MENU_SORT_BY_EPISODE_ASC:
				...
				ed.putInt(AbstractManager.PREF_SORT_BY_PREFIX + AbstractManager.PREF_SORT_KEY_EPISODE,
						SortType.EPISODE_NUM);
				ed.putString(AbstractManager.PREF_SORT_ORDER_PREFIX + AbstractManager.PREF_SORT_KEY_EPISODE,
						SortType.ORDER_ASC);
				ed.commit();
				fetch();
				break;
			...
			case MENU_SORT_BY_RATING_DESC:
				...
				ed.putInt(AbstractManager.PREF_SORT_BY_PREFIX + AbstractManager.PREF_SORT_KEY_EPISODE,
						SortType.EPISODE_RATING);
				ed.putString(AbstractManager.PREF_SORT_ORDER_PREFIX + AbstractManager.PREF_SORT_KEY_SHOW,
						SortType.ORDER_DESC);
				ed.commit();
				fetch();
				break;
	...
	private class EpisodeAdapter extends ArrayAdapter<Episode> {
		...
		public View getView(int position, View convertView, ViewGroup parent) {
			...
			if (mRecentEpisodes) {
				...
				view.subtitle = episode.season + "x" + (episode.episode < 10 ? "0" : "") + episode.episode + ". " +
						episode.title;
			}
