...
public abstract class ImportUtilities {
	...
	public static Bitmap addCoverToCache(ICoverArt cover, Bitmap bitmap, int thumbSize) {
		...
		for (int currentThumbSize : ThumbSize.values()) {
			...
			try {
				...
				Dimension targetDim = ThumbSize.getTargetDimension(currentThumbSize, mediaType, source.getWidth(),
						source.getHeight());
	...
	public static int calculateSampleSize(BitmapFactory.Options options, Dimension targetDimension) {
		...
		Boolean scaleByHeight = Math.abs(options.outHeight - targetDimension.y) >= Math.abs(options.outWidth -
				targetDimension.x);
		if (options.outHeight * options.outWidth * 2 >= 200 * 200 * 2) {
			// Load, scaling to smallest power of 2 that'll get it <= desired dimensions
			double sampleSize = scaleByHeight ? options.outHeight / targetDimension.y : options.outWidth /
					targetDimension.x;
