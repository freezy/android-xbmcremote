...
//		setOnFocusChangeListener(this);
	...
	void addTab(SlidingTabSpec newTab) {
		...
		if (bgImg.getLayoutParams() == null) {
			final LinearLayout.LayoutParams lp = new LayoutParams(ViewGroup.LayoutParams.WRAP_CONTENT,
...
/*		StringBuilder sb = new StringBuilder();
		...
		for (int i = 0; i < mNumTabs; i++) {
			...
			sb.append("[" + img.getPaddingLeft() + "|" + img.getDrawable().getIntrinsicWidth() + "|" + img
			.getPaddingRight() + "] ");
		}
...
//			Log.i("snap", "Snapping to " + index + "(" + snapPos + " / " + backSnapPos + ")");
			...
			final int snapPos = bgImg.getCenteredPosition() - Math.round((float) mSlider.getBackground()
					.getIntrinsicWidth() / 2.0f) + SHADOW_PADDING;
	...
	private void updateLayoutDimensions() {
		if (mSliderMoveWidth == 0) {
			mSliderMoveWidth = mOuterLayout.getWidth() - mSlider.getBackground().getIntrinsicWidth() + (2 *
					SHADOW_PADDING);
		}
		if (mBackMoveWidth == 0) {
			mBackMoveWidth = mInverseSliderWidth - mOuterLayout.getWidth() + mOuterLayout.getPaddingRight() +
					mOuterLayout.getPaddingLeft();// - (2 * SHADOW_PADDING);
		}
...
/*	@Override
	...
	static interface OnTabSelectionChanged {
		 ...
		 * @param clicked  whether the selection changed due to a touch/click or due
			...
			switch (event.getAction()) {
				...
				case MotionEvent.ACTION_MOVE:
					...
					if (sliderAbsPos >= mOuterLayout.getLeft() + mOuterLayout.getPaddingLeft() &&
							sliderAbsPos <= mOuterLayout.getWidth() - mOuterLayout.getPaddingRight() - mSlider
									.getWidth() + (2 * SHADOW_PADDING)) {

						moveSlider(sliderAbsPos);
						moveBackground(bgRelPos);

						// find out nearest background icon
						final int slPos = mSlider.getLeft() + (int) Math.round((float) mSlider.getWidth() / 2.0f) -
								SHADOW_PADDING;
						int minVal = -1;
