...
public class ConnectionLG {
	...
	/**
	 * Path to XBMC's HTTP API script
	 */
	private static final String XBMC_HTTP_BOOTSTRAP =  "/xbmcCmds/xbmcHttp";
	
	/**
	 * Complete URL without any attached command parameters
	 */
	 ...
	 */
	private final INotifiableManager mManager;
	 ...
	 */
	public ConnectionLG(String host, int port, String username, String password, int timeout, INotifiableManager manager) {
		StringBuilder sb = new StringBuilder();
		
		mAuthenticator = new HttpAuthenticator(username, password);
	...
	public synchronized String query(String method, String parameters) {
		...
		try {
			...
			final BufferedReader rd = new BufferedReader(new InputStreamReader(urlConnection.getInputStream()), 8192);
			...
			try {
				if(urlConnection != null) {
					if(((HttpURLConnection)urlConnection).getResponseCode() == HttpURLConnection.HTTP_UNAUTHORIZED) {
...
//		return mBaseURL + XBMC_HTTP_BOOTSTRAP + "?command=" + method + "(" + encodedParameter + ")";
	...
	public synchronized boolean isConnected() {
		try {
			...
			mUrlConnection = query.openConnection();
			mUrlConnection.setConnectTimeout(CONNECTION_TIMEOUT);
			mUrlConnection.setReadTimeout(mTimeout);
			
			mAuthenticator.resetCounter();
			
			final BufferedReader rd = new BufferedReader(new InputStreamReader(mUrlConnection.getInputStream()));
			final StringBuilder sb = new StringBuilder();
