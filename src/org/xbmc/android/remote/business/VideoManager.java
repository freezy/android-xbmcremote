...
public class VideoManager extends AbstractManager implements IVideoManager,
	 ...
	 *            Movie
	 */
	public void updateMovieDetails(final DataResponse<Movie> response,
			final Movie movie, final Context context) {
		mHandler.post(new Command<Movie>(response, this) {
			@Override
			public void doRun() throws Exception {
				response.value = video(context).updateMovieDetails(
						VideoManager.this, movie);
			}
		});
	}
		...
		mHandler.post(new Command<ArrayList<Movie>>(response, this) {
			...
			public void doRun() throws Exception {
				response.value = video(context).getMovies(VideoManager.this,
						getSort(SortType.TITLE), getHideWatched(context));
			}
		});
	}
	...
	public ArrayList<Movie> getMovies(final Context context) {
		try {
			return video(context).getMovies(VideoManager.this,
					getSort(SortType.TITLE), getHideWatched(context));
	...
	public void getMovies(final DataResponse<ArrayList<Movie>> response,
		...
		mHandler.post(new Command<ArrayList<Movie>>(response, this) {
			@Override
			public void doRun() throws Exception {
				response.value = video(context)
						.getMovies(VideoManager.this, actor,
								getSort(SortType.TITLE),
	...
	public void getMovies(final DataResponse<ArrayList<Movie>> response,
		...
		mHandler.post(new Command<ArrayList<Movie>>(response, this) {
			...
			public void doRun() throws Exception {
				response.value = video(context)
						.getMovies(VideoManager.this, genre,
								getSort(SortType.TITLE),
								getHideWatched(context));
			}
		});
	...
	public void getActors(DataResponse<ArrayList<Actor>> response,
		...
		mHandler.post(new Command<ArrayList<Actor>>(response, this) {
			@Override
			public void doRun() throws Exception {
				mResponse.value = video(context).getActors(VideoManager.this);
			}
	...
	public void getMovieActors(final DataResponse<ArrayList<Actor>> response,
		...
		mHandler.post(new Command<ArrayList<Actor>>(response, this) {
			@Override
			public void doRun() throws Exception {
				mResponse.value = video(context).getMovieActors(
						VideoManager.this);
			}
		});
	}
	...
	public void getTvShowActors(final DataResponse<ArrayList<Actor>> response,
		...
		mHandler.post(new Command<ArrayList<Actor>>(response, this) {
			@Override
			public void doRun() throws Exception {
				response.value = video(context).getTvShowActors(
						VideoManager.this);
			}
		});
	}
	...
	public void getMovieGenres(final DataResponse<ArrayList<Genre>> response,
		...
		mHandler.post(new Command<ArrayList<Genre>>(response, this) {
			@Override
			public void doRun() throws Exception {
				response.value = video(context).getMovieGenres(
						VideoManager.this, getSort(SortType.GENRE));
			}
		});
	}
	...
	public void getTvShowGenres(DataResponse<ArrayList<Genre>> response,
		...
		mHandler.post(new Command<ArrayList<Genre>>(response, this) {
			@Override
			public void doRun() throws Exception {
				mResponse.value = video(context).getTvShowGenres(
						VideoManager.this);
			}
		});
	}
	 ...
	 */
	public void setPlaylistVideo(final DataResponse<Boolean> response,
			final int position, final Context context) {
		mHandler.post(new Command<Boolean>(response, this) {
			public void doRun() throws Exception {
				response.value = video(context).setPlaylistPosition(
						VideoManager.this, position);
			}
		});
	}

	/**
	 * Returns an array of videos on the playlist. Empty array if nothing is
	 * playing.
	 * 
	 * @param response
	 *            Response object
	...
	/**
	 * Returns the position of the currently playing video in the playlist.
	 * First position is 0.
	 * 
	 * @param response
	 *            Response object
	...
	public void getPlaylistPosition(final DataResponse<Integer> response,
		...
		mHandler.post(new Command<Integer>(response, this) {
			public void doRun() throws Exception {
				response.value = video(context).getPlaylistPosition(
						VideoManager.this);
			}
		});
	}
	...
	public void removeFromPlaylist(final DataResponse<Boolean> response,
		...
		mHandler.post(new Command<Boolean>(response, this) {
			public void doRun() throws Exception {
				response.value = video(context).removeFromPlaylist(
						VideoManager.this, position);
			}
		});
	}
	...
	public void downloadCover(DataResponse<Bitmap> response, ICoverArt cover,
		...
		response.value = ClientFactory.getVideoClient(this, context).getCover(
