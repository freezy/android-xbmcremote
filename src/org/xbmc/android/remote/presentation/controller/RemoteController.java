...
public class RemoteController extends AbstractController implements
		INotifiableController, IController {
	...
	public RemoteController(Context context) {
		...
		mInfoManager.getGuiSettingInt(new DataResponse<Integer>() {
			...
			// Log.i("RemoteController", "Saving previous value " +
			// GuiSettings.getName(GuiSettings.Services.EVENT_SERVER_INITIAL_DELAY)
			// + " = " + value);
			// }
			// });
			// }
		...
		IGestureListener listener = new IGestureListener() {
			...
			public void onHorizontalMove(int value) {
				...
				if (value == 0) {
					...
					mGestureThread.setLevel(value,
							value > 0 ? GestureThread.ACTION_RIGHT
									: GestureThread.ACTION_LEFT);
				}
			}
			...
			public void onVerticalMove(int value) {
				...
				if (value == 0) {
					...
					mGestureThread.setLevel(value,
							value > 0 ? GestureThread.ACTION_DOWN
									: GestureThread.ACTION_UP);
				}
			}
				...
				if (amount != 0) {
					if (!mScrolling) {
						Log.i(TAG,
								"Setting "
										+ GuiSettings
												.getName(GuiSettings.Services.EVENT_SERVER_INITIAL_DELAY)
										+ " = " + 25);
						mInfoManager
								.setGuiSettingInt(
										new DataResponse<Boolean>(),
										GuiSettings.Services.EVENT_SERVER_INITIAL_DELAY,
										25, context);
					}
					mEventClientManager.sendButton("XG", button, true, true,
							false, (short) (amount * 65535), (byte) 0);
					mScrolling = true;
				} else {
					mEventClientManager.sendButton("XG", button, false, false,
							false, (short) 0, (byte) 0);
					Log.i(TAG,
							"Restoring "
									+ GuiSettings
											.getName(GuiSettings.Services.EVENT_SERVER_INITIAL_DELAY)
									+ " = " + mEventServerInitialDelay);
					mInfoManager.setGuiSettingInt(new DataResponse<Boolean>(),
							GuiSettings.Services.EVENT_SERVER_INITIAL_DELAY,
							mEventServerInitialDelay, context);
					mScrolling = false;
				}
			}
			...
			public void onSelect() {
				mEventClientManager.sendButton("R1", ButtonCodes.REMOTE_SELECT,
						false, true, false, (short) 0, (byte) 0);
			}

			public void onScrollDown() {
				Log.d(TAG, "onScrollDown()");
				mEventClientManager.sendButton("KB",
						ButtonCodes.KEYBOARD_PAGEDOWN, false, true, true,
						(short) 0, (byte) 0);
			}
			...
			public void onScrollUp() {
				...
				mEventClientManager.sendButton("KB",
						ButtonCodes.KEYBOARD_PAGEUP, false, true, true,
						(short) 0, (byte) 0);
			}

			public void onBack() {
				mEventClientManager.sendButton("R1", ButtonCodes.REMOTE_BACK,
						false, true, true, (short) 0, (byte) 0);
			}

			public void onInfo() {
				mEventClientManager.sendButton("R1", ButtonCodes.REMOTE_INFO,
						false, true, true, (short) 0, (byte) 0);
			}
			...
			public void onTitle() {
				mEventClientManager.sendButton("R1", ButtonCodes.REMOTE_TITLE,
						false, true, true, (short) 0, (byte) 0);
			}
		};
		return listener;
		...
		public void run() {
			...
			while (!mQuit) {
				try {
					switch (mAction) {
					case ACTION_UP:
						mEventClient.sendButton("R1", ButtonCodes.REMOTE_UP,
								false, true, true, (short) 0, (byte) 0);
						break;
					case ACTION_RIGHT:
						mEventClient.sendButton("R1", ButtonCodes.REMOTE_RIGHT,
								false, true, true, (short) 0, (byte) 0);
						break;
					case ACTION_DOWN:
						mEventClient.sendButton("R1", ButtonCodes.REMOTE_DOWN,
								false, true, true, (short) 0, (byte) 0);
						break;
					case ACTION_LEFT:
						mEventClient.sendButton("R1", ButtonCodes.REMOTE_LEFT,
								false, true, true, (short) 0, (byte) 0);
						break;
					}
	...
	public boolean onKeyDown(int keyCode, KeyEvent event) {
		...
		case KeyEvent.KEYCODE_VOLUME_UP:
			mEventClientManager.sendButton("R1",
					ButtonCodes.REMOTE_VOLUME_PLUS, false, true, true,
					(short) 0, (byte) 0);
			return true;
		case KeyEvent.KEYCODE_VOLUME_DOWN:
			mEventClientManager.sendButton("R1",
					ButtonCodes.REMOTE_VOLUME_MINUS, false, true, true,
					(short) 0, (byte) 0);
			return true;
	...
	private boolean onDirectionalPadDown(int keyCode) {
		...
		if (newstamp - mTimestamp > DPAD_DOWN_MIN_DELTA_TIME) {
			...
			switch (keyCode) {
			case KeyEvent.KEYCODE_DPAD_DOWN:
				mEventClientManager.sendButton("R1", ButtonCodes.REMOTE_DOWN,
						false, true, true, (short) 0, (byte) 0);
				return true;
			case KeyEvent.KEYCODE_DPAD_UP:
				mEventClientManager.sendButton("R1", ButtonCodes.REMOTE_UP,
						false, true, true, (short) 0, (byte) 0);
				return true;
			case KeyEvent.KEYCODE_DPAD_LEFT:
				mEventClientManager.sendButton("R1", ButtonCodes.REMOTE_LEFT,
						false, true, true, (short) 0, (byte) 0);
				return true;
			case KeyEvent.KEYCODE_DPAD_RIGHT:
				mEventClientManager.sendButton("R1", ButtonCodes.REMOTE_RIGHT,
						false, true, true, (short) 0, (byte) 0);
				return true;
			case KeyEvent.KEYCODE_DPAD_CENTER:
				mEventClientManager.sendButton("R1", ButtonCodes.REMOTE_SELECT,
	...
	public boolean onTrackballEvent(MotionEvent event) {
		if (event.getAction() == MotionEvent.ACTION_DOWN)
			...
			// check when the last trackball move happened to avoid too speedy
			// selections
			long newstamp = System.currentTimeMillis();
			if (newstamp - mTimestamp > MOTION_EVENT_MIN_DELTA_TIME) {
				mTimestamp = newstamp;
				if (Math.abs(event.getX()) > MOTION_EVENT_MIN_DELTA_POSITION) {
					return keyboardAction(event.getX() < 0 ? ButtonCodes.KEYBOARD_LEFT
							: ButtonCodes.KEYBOARD_RIGHT);
				} else if (Math.abs(event.getY()) > MOTION_EVENT_MIN_DELTA_POSITION) {
					return keyboardAction(event.getY() < 0 ? ButtonCodes.KEYBOARD_UP
							: ButtonCodes.KEYBOARD_DOWN);
				}
			}
		}
		return true;
	...
	public boolean onCreateOptionsMenu(Menu menu) {
		...
		menu.add(0, MENU_SWITCH_GESTURE, 0, "Gesture mode").setIcon(
				R.drawable.menu_gesture_mode);
		menu.add(0, MENU_ENTER_TEXT, 0, "Text Entry").setIcon(
				R.drawable.menu_text_entry);
		menu.add(0, MENU_XBMC_EXIT, 0, "Exit XBMC").setIcon(
				R.drawable.menu_xbmc_exit);
		menu.add(0, MENU_XBMC_S, 0, "Press \"S\"").setIcon(
				R.drawable.menu_xbmc_s);
		return true;
	}

	// Need Context passed in because this can be called at times when mActivity
	// is null.
	public Dialog onCreateDialog(int id, final Context context) {
		...
		case DIALOG_SENDTEXT:
			...
			Button sendbutton = (Button) dialog
					.findViewById(R.id.sendtext_button_send);
			sendbutton.setOnClickListener(new OnClickListener() {
				public void onClick(View v) {
					EditText text = (EditText) v.getRootView().findViewById(
							R.id.sendtext_text);
					mControl.sendText(new DataResponse<Boolean>(), text
							.getText().toString(), context);
					text.setText("");
				}
			});
			Button donebutton = (Button) dialog
					.findViewById(R.id.sendtext_button_done);
			...
			Button cancelbutton = (Button) dialog
					.findViewById(R.id.sendtext_button_cancel);
	...
	public boolean onOptionsItemSelected(MenuItem item) {
		...
		case MENU_SWITCH_GESTURE:
			...
			break;
		case MENU_XBMC_EXIT:
			mEventClientManager.sendButton("R1", ButtonCodes.REMOTE_POWER,
					false, true, true, (short) 0, (byte) 0);
			break;
		case MENU_XBMC_S:
			mEventClientManager.sendButton("KB", "S", false, true, true,
					(short) 0, (byte) 0);
			break;
	...
	private boolean keyboardAction(String button) {
		mEventClientManager.sendButton("KB", button, false, true, true,
				(short) 0, (byte) 0);
		return true;
	}
	 ...
	 * @param action
	 *            Action string
	 * @param resourceButtonUp
	 *            Resource ID of the button up image
	 * @param resourceButtonDown
	...
	private class OnRemoteAction implements OnTouchListener {
		...
		public boolean onTouch(View v, MotionEvent event) {
			if (event.getAction() == MotionEvent.ACTION_DOWN) {
				...
				mEventClientManager.sendButton("R1", mAction,
						!prefs.getBoolean("setting_send_repeats", false), true,
						true, (short) 0, (byte) 0);

				if (prefs.getBoolean("setting_send_repeats", false)
						&& !prefs
								.getBoolean("setting_send_single_click", false)) {

					if (tmrKeyPress != null) {
						tmrKeyPress.cancel();
					}

					int RepeatDelay = Integer.parseInt(prefs.getString(
							"setting_repeat_rate", "250"));

					tmrKeyPress = new Timer();
					tmrKeyPress.schedule(new KeyPressTask(mAction),
				...
				mEventClientManager.sendButton("R1", mAction, false, false,
						true, (short) 0, (byte) 0);
	...
	private class KeyPressTask extends TimerTask {
		...
		public void run() {
			if (mKeyPressAction.length() > 0) {
				mEventClientManager.sendButton("R1", mKeyPressAction, false,
