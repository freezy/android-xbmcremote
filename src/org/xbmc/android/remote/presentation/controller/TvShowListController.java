...
//		final SharedPreferences.Editor ed;
	...
	private class TvShowAdapter extends ArrayAdapter<TvShow> {
		...
		public View getView(int position, View convertView, ViewGroup parent) {
			...
			view.title = tvShow.title;
			view.subtitle = tvShow.genre;
			view.bottomtitle = tvShow.numEpisodes + " episodes";
			view.bottomright = String.valueOf((float)Math.round(tvShow.rating * 10) / 10f);
			view.subtitleRight = tvShow.network;
			
			if (mLoadCovers) {
				view.getResponse().load(tvShow);
			}
			return view;
		}
	}
