...
public class FileLocation implements INamedResource {
		...
		if (path.startsWith("rar://") || path.startsWith("zip://")) {
			...
			if (isDirectory) {
				decoded = URLDecoder.decode(
						path.substring(0, path.length() - 1)).replaceAll(
						"\\\\", "/");
				isArchive = true;
			} else {
			...
			if (matcher.matches()) {
				...
				pattern = Pattern
						.compile(
								".*shoutcast\\.com.*tunein-station\\.pls\\?id=([0-9]+)",
								Pattern.CASE_INSENSITIVE);
			...
			if (matcher.matches()) {
				...
				pattern = Pattern.compile(".*globaltags/([^/]+)",
						Pattern.CASE_INSENSITIVE);
				matcher = pattern.matcher(path);
				if (matcher.matches()) {
	 ...
	 * 
	 * @param line
	 *            raw line, either path only or name and path, separated by
	 *            Connection.VALUE_SEP.
	 */
